...
import { writeToString, countTriples } from './graph-helpers';
...
const batchSize = parseInt(process.env.DELETE_BATCH_SIZE) || 1000;
...
export default async function enrich(submissionDocument) {
  ...
  const countTmpGraph = await countTriples(tmpGraph);
  ...
  try {
    if (countTmpGraph > 0) {
      ...
      while (offset < countTmpGraph) {
        await update(deleteQuery);
        offset = offset + batchSize;
        console.log(`Deleted ${offset < countTmpGraph ? offset : countTmpGraph}/${countTmpGraph} triples from graph <${tmpGraph}>`);
      }
    }
  } catch (e) {
