...
const EREDIENSTEN_AND_CENTRALE_BESTUREN_RO_SCHEME =
...
async function addFilteredEredienstenAndCentraleBesturenRO(submissionDocument, store) {
  ...
  for (const binding of results.results.bindings) {
    ...
    store.add(
      sym(subject),
      SKOS("inScheme"),
      sym(EREDIENSTEN_AND_CENTRALE_BESTUREN_RO_SCHEME),
      sym(defaultGraph)
    );
    store.add(sym(subject), SKOS("prefLabel"), label , sym(defaultGraph));
  }
  ...
  SELECT DISTINCT ?erediensten ?label ?classificatie WHERE {
    GRAPH ?g {
      ...
      ?erediensten <http://data.vlaanderen.be/ns/besluit#classificatie> ?classificatie;
          <http://www.w3.org/2004/02/skos/core#prefLabel> ?label.
      FILTER NOT EXISTS {
        ?erediensten <http://data.vlaanderen.be/ns/besluit#classificatie> <http://data.vlaanderen.be/id/concept/BestuurseenheidClassificatieCode/f9cac08a-13c1-49da-9bcb-f650b0604054>
      }
    }
  }
  `;
