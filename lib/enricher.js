...
async function expandDocumentType(submissionDocument, tmpGraph) {
  console.log(`Add expanded document types for submission document ${submissionDocument} to meta graph`);
  try {
    let result = await query(`
        ...
        SELECT ?ttl
        WHERE {
          ?submission terms:subject  ${sparqlEscapeUri(submissionDocument)};
                      nie:hasPart    ?one .
          ?two        nie:dataSource ?one .
          ?ttl        nie:dataSource ?two .
        }
        LIMIT 1
    `);

    const ttl = new TurtleFile({uri: result.results.bindings.map(binding => binding['ttl'].value)[0]});

    try {
      ttl.read();

      try {
        const types = uniq(ttl
          .match(undefined, RDF('type'), undefined)
          .map(t => ({subject: sparqlEscapeUri(t.subject.value), uri: sparqlEscapeUri(t.object.value)}))
        );
        ...
        for (let type of types) {
          await query(`
            ...
            INSERT {
              ...
              ${type.uri} a skos:Concept, rdfs:Class ;
                    skos:broader+ ?parent .
              ?parent a rdfs:Class .
              BIND(IRI(${type.subject}) as ?subject)
            }`
          );
        }
      } catch (e) {
        console.log(`Something went wrong while inserting the expanded document types for submission document ${submissionDocument}`);
        console.log(`Exception: ${e.stack}`);
      }
    } catch (e) {
      console.log(`Something went wrong while parsing harvested ttl-file ${ttl.uri}`);
      console.log(`Exception: ${e.stack}`);
    }
  } catch (e) {
    console.log(`No harvested file could be found for submission document ${submissionDocument}`);
    console.log(`Exception: ${e.stack}`);
  }
}
