...
const BESTUURSORGAAN_SELECT_CONCEPT_SCHEME =
  "http://data.lblod.info/concept-schemes/481c03f0-d07f-424e-9c2b-8d4cfb141c72";
const TYPE_DOSSIER_CONCEPT_SCHEME =
...
async function addRelevantDossierTypes(submissionDocument, store) {
  ...
  await addTriples(store, {
    WHERE: `${sparqlEscapeUri(
      bestuurseenheid
    )} besluit:classificatie ?classificatie .
            ?s <http://lblod.data.gift/vocabularies/besluit/decidableBy> ?classificatie ;
                skos:inScheme ${sparqlEscapeUri(
                  TYPE_DOSSIER_CONCEPT_SCHEME
                )} .`,
  });
}
...
async function addBestuursorganenForBestuurseenheid(submissionDocument, store) {
  ...
  if (result.results.bindings.length) {
    result.results.bindings.forEach((b) => {
      ...
      store.add(
        sym(BESTUURSORGAAN_SELECT_CONCEPT_SCHEME),
        RDF("type"),
        SKOS("ConceptScheme"),
        sym(defaultGraph)
      );
...
`;
  ...
  for (const binding of results.results.bindings) {
    ...
    store.add(sym(subject), SKOS("prefLabel"), sym(label), sym(defaultGraph));
  }
