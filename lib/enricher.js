...
export default async function enrich(submissionDocument) {
  ...
  await addManagedErediensten(submissionDocument, store);
...
/**
 * Creates a concept scheme that contains all the "eredienstbesturen" that are managed by the "centraal eredienstbestuur" that created the submission document. 
 * @param {string} submissionDocument 
 * @param {rdflibStore} store 
 * @returns {void}
 */
async function addManagedErediensten(submissionDocument, store) {
  console.log(`Adding linked worship-services to meta graph`);
  const bestuurseenheid = await getBestuurseenheidFor(submissionDocument);
