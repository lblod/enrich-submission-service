...
const TOEZICHT_CONCEPT_SCHEMES = [
  ...
  'http://lblod.data.gift/concept-schemes/c7d95519-9ef1-402b-b824-56ec3b1a0f61', // Type Kerkenbeleidsplan
...
const EREDIENSTEN_AND_CENTRALE_BESTUREN_FILTERED_RO_SCHEME =
  "http://lblod.data.gift/concept-schemes/7904627a-4e62-42a1-a299-9fe39a1dda6b";
const EREDIENSTEN_SUBORGANIZATIONS_SCHEME =
  "http://lblod.data.gift/concept-schemes/04c8d012-cc8e-4f68-9908-69cc38adf040";
...
export default async function enrich(submissionDocument) {
  ...
  await addFilteredEredienstenAndCentraleBesturenRO(submissionDocument, store);
  await addFilteredEredienstenAndCentraleBesturenGOAndPO(submissionDocument, store);
  await addGovernedErediensten(submissionDocument, store);
...
async function addRelevantDossierTypes(submissionDocument, store) {
  ...
  console.log(`Found bestuurseenheid ${bestuurseenheid}`);
  await addTriples(store, {
    WHERE: `${sparqlEscapeUri(bestuurseenheid)}
              besluit:classificatie ?classificatie .
            
            ?besluitDocumentType
              a skos:Concept ;
              <http://lblod.data.gift/vocabularies/besluit/notificationRule> ?rule ;
              skos:inScheme ${sparqlEscapeUri(TYPE_DOSSIER_CONCEPT_SCHEME)} .

            ?rule
              a <http://lblod.data.gift/vocabularies/notification/NotificationRule> ;
              <http://lblod.data.gift/vocabularies/besluit/decidableBy> ?classificatie ;
              <http://lblod.data.gift/vocabularies/besluit/obligationToReport>
                "true"^^xsd:boolean .

            OPTIONAL { ?rule <http://schema.org/validFrom> ?validFrom . }
            OPTIONAL { ?rule <http://schema.org/validThrough> ?validThrough . }

            FILTER (
              ?validFrom <= NOW() &&
              ( !BOUND(?validThrough) || ?validThrough >= NOW() )
            )

            BIND(?besluitDocumentType AS ?s)`
  });
}
...
async function addFilteredEredienstenAndCentraleBesturenRO(submissionDocument, store) {
  console.log(`Adding linked worship-services RO to meta graph`);
  const bestuurseenheid = await getBestuurseenheidFor(submissionDocument);
  const q = `
  SELECT DISTINCT ?erediensten ?label ?classificatie ?kboNumber WHERE {
  ...
  for (const binding of results.results.bindings) {
    ...
    store.add(
      sym(subject),
      SKOS("inScheme"),
      sym(EREDIENSTEN_AND_CENTRALE_BESTUREN_FILTERED_RO_SCHEME),
      sym(defaultGraph)
    );
    store.add(sym(subject), SKOS("prefLabel"), combinedLabel , sym(defaultGraph));
  }
...
/**
 * Creates a concept scheme that contains all the "eredienstbesturen" that are governed by the "centraal eredienstbestuur" that created the submission document.
 * @param {string} submissionDocument
 * @param {rdflibStore} store
 * @returns {void}
 */
async function addGovernedErediensten(submissionDocument, store) {
  console.log(`Adding linked worship-services to meta graph`);
  const bestuurseenheid = await getBestuurseenheidFor(submissionDocument);
  const q = `SELECT DISTINCT ?eredienst ?label ?classificatie WHERE {
    GRAPH ?g {
      ${sparqlEscapeUri(bestuurseenheid)} a <http://data.lblod.info/vocabularies/erediensten/CentraalBestuurVanDeEredienst> ;
        <http://www.w3.org/ns/org#hasSubOrganization> ?eredienst .

      ?eredienst <http://data.vlaanderen.be/ns/besluit#classificatie> ?classificatie ;
          <http://www.w3.org/2004/02/skos/core#prefLabel> ?label .
    }
  }`;
  ...
  const results = await query(q);
  for (const binding of results.results.bindings) {
    const subject = binding.eredienst.value;
    const type = binding.classificatie.value;
    const label = binding.label.value;
    ...
    store.add(
      sym(subject),
      SKOS("inScheme"),
      sym(EREDIENSTEN_SUBORGANIZATIONS_SCHEME),
      sym(defaultGraph)
    );
