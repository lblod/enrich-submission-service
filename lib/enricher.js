...
const EREDIENSTEN_CONCEPT_SCHEME =
  "http://lblod.data.gift/concept-schemes/fa83ff76-2f1d-496c-8f93-b498a02d550f";
const EREDIENSTEN_FILTERED_SCHEME = 
  "http://lblod.data.gift/concept-schemes/362a6a78-6431-4d0a-b20d-22f1faca4130";
const EREDIENSTEN_RO_SCHEME =
  "http://lblod.data.gift/concept-schemes/936272b5-020a-43f9-8115-503e83bb0631";
/**
...
export default async function enrich(submissionDocument) {
  ...
  await addErediensten(store);
  await addFilteredErediensten(submissionDocument, store);
...
async function addErediensten(store) {
  console.log(`Adding erediensten to meta graph`);
  const q = `
  SELECT DISTINCT ?subject ?label ?type WHERE {
    GRAPH ?g {
      ?subject <http://data.vlaanderen.be/ns/besluit#classificatie>  ?type;
        <http://www.w3.org/2004/02/skos/core#prefLabel> ?label .
        FILTER (?type IN (<http://data.vlaanderen.be/id/concept/BestuurseenheidClassificatieCode/66ec74fd-8cfc-4e16-99c6-350b35012e86>, <http://data.vlaanderen.be/id/concept/BestuurseenheidClassificatieCode/f9cac08a-13c1-49da-9bcb-f650b0604054> ) )
    }
  }
`;
  const results = await query(q);
  for (const binding of results.results.bindings) {
    ...
    store.add(
      sym(subject),
      SKOS("inScheme"),
      sym(EREDIENSTEN_CONCEPT_SCHEME),
      sym(defaultGraph)
    );
...
async function addFilteredErediensten(submissionDocument, store) {
  ...
  for (const binding of results.results.bindings) {
    ...
    store.add(
      sym(subject),
      SKOS("inScheme"),
      sym(EREDIENSTEN_FILTERED_SCHEME),
      sym(defaultGraph)
    );
    store.add(sym(subject), SKOS("prefLabel"), label , sym(defaultGraph));
  }
