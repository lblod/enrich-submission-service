...
import {parse as rdflibParse, serialize as rdflibSerialize, Namespace} from 'rdflib';
...
export function serialize(store) {
  return rdflibSerialize(defaultGraph, store, undefined, 'application/n-triples');
}
...
export async function addTriples(store, {PREFIXES = [], WHERE}) {
  ...
  if (count > 0) {
    ...
    let offset = 0;
    const query = `
      ${(PREFIXES.map(prefix => `PREFIX ${prefix}`).join('\n      '))}
      SELECT ?s ?p ?o
      WHERE {
