...
async function deleteSubmissionDocument(uuid) {
  ...
  if (submissionDocument) {
    ...
    if (status == SENT_STATUS) {
      ...
      // Donc là il faut :
      // - supprimer le document ainsi que ces properties
      // - supprimer les fichiers associés dans le store
      // - supprimer les fichiers dans app/... : s'inspirer du file-helpers :)
      // --> Faire ça dans le sens inverse sinon ça va râler haha
      ...
      deleteTtlFile(additionsFile);
      deleteTtlFile(removalsFile);
      deleteTtlFile(metaFile);
      deleteTtlFile(sourceFile);
