...
export async function deleteSubmissionDocument(uuid) {
  ...
  if (submissionDocument.value) {
    ...
    if (status.value == env.SENT_STATUS) {
      ...
      if (additionsFile) fil.removeFromLogicalFile(additionsFile);
      if (removalsFile) fil.removeFromLogicalFile(removalsFile);
      if (metaFile) fil.removeFromLogicalFile(metaFile);
      if (sourceFile) fil.removeFromLogicalFile(sourceFile);
