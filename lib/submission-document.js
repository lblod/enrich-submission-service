...
export async function deleteSubmissionDocument(uuid) {
  ...
  if (submissionDocument.value) {
    ...
    if (status.value == env.SENT_STATUS) {
      ...
      if (additionsFile) fil.removeFromLogicalFile(namedNode(additionsFile));
      if (removalsFile) fil.removeFromLogicalFile(namedNode(removalsFile));
      if (metaFile) fil.removeFromLogicalFile(namedNode(metaFile));
      if (sourceFile) fil.removeFromLogicalFile(namedNode(sourceFile));
...
export async function calculateMetaSnapshot(submissionDocument) {
  const content = await enrich(namedNode(submissionDocument));
...
export async function calculateActiveForm(
  ...
  return fil.loadFromPhysicalFile(namedNode(formFile));
}
...
async function getHarvestedData(submissionDocument) {
  ...
  if (result.results.bindings.length) {
    ...
    return fil.loadFromLogicalFile(namedNode(file));
  }
}
...
async function getPart(submissionDocument, fileType) {
  ...
  if (file) return fil.loadFromLogicalFile(namedNode(file));
}
async function getPartWithoutLogical(submissionDocument, fileType) {
  const file = await getFileResource(submissionDocument, fileType);
  if (file) return fil.loadFromPhysicalFile(namedNode(file));
}
...
async function savePart(submissionDocument, content, fileType) {
  ...
  if (!result.results.bindings.length) {
    ...
    `);
    const graph = namedNode(result.results.bindings[0]?.g?.value);
    const { logicalFile } = await fil.createFromContent(content, graph);
    await updateSudo(`
      PREFIX dct: <http://purl.org/dc/terms/>
      PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    ...
    await fil.updateContentForLogicalFile(namedNode(logicalFile), content);
    return logicalFile;
  }
}
