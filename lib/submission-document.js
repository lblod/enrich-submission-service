...
async function getSubmissionDocument(uuid) {
  ...
  if (submissionDocument) {
    ...
    if (status == CONCEPT_STATUS || status == SUBMITABLE_STATUS) {
      ...
      const form = await calculateActiveForm(submissionDocument);
 ...
 */
async function calculateActiveForm(submissionDocument) {
  if(ACTIVE_FORM_FILE) {
    await saveForm(submissionDocument)
    return await getFileContent(ACTIVE_FORM_FILE);
  } else {
    throw new Error(
      "For this service to work an environment variable ACTIVE_FORM_FILE should be configured.\n" +
      "This variable is used to obtain the form configuration.")
  }
}
...
async function saveForm(submissionDocument){
  await updateSudo(`
      PREFIX dct: <http://purl.org/dc/terms/>
      ...
      INSERT {
        GRAPH ?g {
          ${sparqlEscapeUri(submissionDocument)} dct:source ${sparqlEscapeUri(ACTIVE_FORM_FILE)} .
        }
        GRAPH ${sparqlEscapeUri(FILE_GRAPH)} {
          ${sparqlEscapeUri(ACTIVE_FORM_FILE)} dct:type ${sparqlEscapeUri(FORM_FILE_TYPE)} .
        }
      } WHERE {
        GRAPH ?g {
