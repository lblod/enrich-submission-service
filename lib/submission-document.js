...
async function getSubmissionDocument(uuid) {
  ...
  if (submissionDocument) {
    ...
    if (status == CONCEPT_STATUS || status == SUBMITABLE_STATUS) {
      ...
      const form = getSubmittedForm(submissionDocument);
...
/**
 * Update the additions and removals of the submission document with the given id
*/
async function updateSubmissionDocument(uuid, { additions, removals }) {
  const { submissionDocument, status } = await getSubmissionDocumentById(uuid);
...
async function getActiveForm() {
  return fs.readFileSync(FORM_TTL, 'utf-8');
}

async function calculateMetaSnapshot(submissionDocument) {
  // TODO construct Meta TTL:
  // - codelists
  // - skos expansion
  // - bestuursorganen for bestuurseenheid
  const content = '';
  await this.saveMeta(submissionDocument, content);
}
...
*/
async function getSubmittedForm(submissionDocument) {
