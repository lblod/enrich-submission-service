...
export async function deleteSubmissionDocument(uuid) {
  ...
  if (submissionDocument) {
    ...
    if (status == SENT_STATUS) {
      console.log(
        `Form with uuid ${uuid} is in sent status and can\'t be deleted.`
      );
    } else {
...
async function getSubmissionDocumentById(uuid) {
  ...
  if (result.results.bindings.length) {
    return {
      submissionDocument:
        result.results.bindings[0]['submissionDocument'].value,
  ...
  await updateSudo(`
      ...
      DELETE {
        ...
        GRAPH ${sparqlEscapeUri(PUBLIC_FILES_GRAPH)} {
          ?file
            dct:type ${sparqlEscapeUri(FORM_FILE_TYPE)} .
        }
      }
      ;
      INSERT {
