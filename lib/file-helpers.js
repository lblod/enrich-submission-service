 ...
 */
export async function getFileContent(logicalFileUri) {
...
export async function getFileContentPhysical(physicalFileUri) {
  console.log(`Getting contents of file ${physicalFileUri}`);
  const path = physicalFileUri.replace('share://', '/share/');
  const content = await fs.readFile(path, 'utf8');
  return content;
      ...
      INSERT {
        GRAPH ?g {
          ${sparqlEscapeUri(physicalUri)}
            ...
            dct:creator ${sparqlEscapeUri(cts.SERVICES.enrichSubmission)} ;
 ...
 */
export async function deleteTtlFile(logicalFile) {
  const response = await querySudo(`
  ...
  try {
    await update(`
      DELETE WHERE {
          ...
          ${sparqlEscapeUri(logicalFile)} ?p2 ?o2 .
      }
    `);
  } catch (e) {
    console.log(
      `Failed to delete TTL resource <${logicalFile}> in triplestore: \n ${e}`
    );
    throw e;
  }
}
